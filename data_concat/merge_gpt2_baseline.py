...
Also supports merging diffusion model metrics from metrics_diff.json.
"""
...
def merge_diffusion_metrics(metrics_file, diffusion_file, model_name, output_file=None):
    """
    Merge specific diffusion model metrics into the main metrics file.
    
    Args:
        metrics_file: Path to the main metrics.json file
        diffusion_file: Path to the metrics_diff.json file
        model_name: Name of the diffusion model to merge (e.g., "diffusion_encoder_embd=256_layer=12_head=8_timesteps=100")
        output_file: Path to save the merged output (defaults to metrics_file)
    """
    # Read both JSON files
    print(f"Reading {metrics_file}...")
    ...
    print(f"Reading {diffusion_file}...")
    with open(diffusion_file, 'r') as f:
        diffusion_metrics = json.load(f)
    
    # Merge the metrics
    print(f"Merging diffusion model '{model_name}' into main metrics...")
    models_added = 0
    models_updated = 0
    
    for task_type, models in diffusion_metrics.items():
        # Only merge if the specific model exists in this task type
        ...
        if model_name in metrics[task_type]:
            print(f"  Updating existing model: {task_type}/{model_name}")
            models_updated += 1
        else:
            print(f"  Adding new model: {task_type}/{model_name}")
            models_added += 1
        ...
        metrics[task_type][model_name] = models[model_name]
